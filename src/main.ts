...
function* stageScript(){
    ...
    enemy1(-25,0,fw.width/3,fw.height/4,0,-10,15,30)
    for(let i = 0; i < 30; i++) yield;
    enemy1(fw.width+25,0,2*fw.width/3,fw.height/4,0,-10,15,0)
    for(let i = 0; i < 60; i++) yield;
    
    enemy2(4*fw.width/10,-45,0,5,20)
    enemy2(5*fw.width/10,-45,0,5,20)
    enemy2(6*fw.width/10,-45,0,5,20)
    for(let i = 0; i < 60; i++) yield;
    ...
    for(let i = 1; i <= 6; i++) {
        enemy4(i*(fw.width+100)/7-50, -45,10)
        yield;yield;
    }
    for(let i = 0; i < 60; i++) yield;
    for(let i = 6; i >= 1; i--) {
    ...
    for(let i = 0; i < 90; i++) yield;
    enemy6(-90,3*fw.height/4,fw.width/4,fw.height/3,30,180)
    for(let i = 0; i < 30; i++) yield;
    enemy6(fw.width/2,-55,fw.width/2,fw.height/4,30,150)
    for(let i = 0; i < 30; i++) yield;
    enemy6(fw.width+90,3*fw.height/4,3*fw.width/4,fw.height/2,30,120)
